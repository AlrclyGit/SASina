...
//    self.tableView.contentInset = UIEdgeInsetsMake(SAStatusCellMargin, 0, 0, 0);
...
- (void)setupUserInfo{
    ...
    [mgr GET:@"https://api.weibo.com/2/users/show.json" parameters:params success:^(AFHTTPRequestOperation *operation, NSDictionary *responseObject) {
        ...
        //设置名字
        SAUser *user = [SAUser objectWithKeyValues:responseObject];
...
/**
 *  UIRefreshControl进入上拉更多状态:加载以前的数据
 */
- (void)loadMoreStatus{
    // 1.请求管理者
    AFHTTPRequestOperationManager *mgr = [AFHTTPRequestOperationManager manager];
...
- (void)setupUnredadCount{
    ...
    //3.发送请求
    [mgr GET:@"https://api.weibo.com/2/remind/unread_count.json" parameters:params success:^(AFHTTPRequestOperation *operation, NSDictionary *responseObject) {
        SALog(@"未读数请求成功——%@",responseObject);
        //获取设置微博末读数
...
#pragma mark - SADropdownMenuDelegate
/**
 * 创建，箭头向上
 */
